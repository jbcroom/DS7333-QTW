...
Now that the data has been broken down into individual record sets for each year, an initial look at the data structure for each individual record set is warranted. This will ensure that data mapped into the individual data sets represents the correct data types for the values that are expected to be pulled from the race webpages. Since all individual record sets are pulled in the same format evaluating the 2000 record set will provide a quick indication that data was mapped correctly.
...
x <- c("Race Year", "Missing Record Count")
...
Evaluation of the race results web page for the 2001 race does confirm that the age entered for this individual was 0. As such this should be treated as an invalid data point, with respect to age. On a positive note, there is only one record in this data set with a zero value for age. 
...
ggplot(women_combined_df, aes(x=Year, y=Ag)) + 
  geom_boxplot(outlier.colour="red", outlier.shape=8,
               outlier.size=4) +
  ggtitle("Women's Racers Ages - All Years") + ylab("Age")
```
...
```{r, echo=FALSE,message=FALSE, warning=FALSE}
  ggplot(women_combined_df) + 
    ...
    geom_vline(aes(xintercept=mean(Ag)), color="red", linetype="dashed", size=1) + xlab("Age")
```

Continuing to evaluate runners ages the data provides an opportunity to evaluate mean age by race year as shown in the table below.
```{r,echo=FALSE,message=FALSE, warning=FALSE}
...
```
This table confirms the distribution plot and box plots above with respect to racers in 2006 skewing much younger for that year. Using the mean age of all races as 32, looking specifically at 2006 it can be seen that of the 5435 racers in 2006, 3784 of them were below the mean race age.
...
```
Digging deeper into the 2006 data specifically, the distribution plot below shows that a large portion of the runners in 2006 were under 18 years of age. There could be many factors that account for this lower age skew.

* Runners could be truly entering at a younger age
* Runners could be providing lower ages
* Data could have been entered incorrectly

Each of these cases could take additional data and understanding of the circumstances to come to a concrete conclusion. 

```{r, echo=FALSE,message=FALSE, warning=FALSE}
  ggplot(women_2006_df) + 
    geom_density(aes(x = women_2006_df$A), alpha = 0.7,color="green", fill="lightgreen") + 
    ggtitle("Women's Racers Age Distribution - 2006") + 
    geom_vline(aes(xintercept=mean(women_2006_df$A)), color="red", linetype="dashed", size=1) + xlab("Age")
```
...
```{r, echo=FALSE,message=FALSE,warning=FALSE}
  ...
  women_combined_df %>% 
    ...
    geom_point()+
    geom_jitter() + 
    ggtitle("Women's Racers Ages - All Years - Scatter Plot") + ylab("Age")
...
Both the scatterplot and table above confirm that the number of racers has continued to increase from 1999 through 2012.
...
The distribution chart below also reaffirms that 2011 was an outlier year.
