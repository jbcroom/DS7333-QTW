...
Viewing the combined elbow plot above it can be seen that for the CD configuration a k=4 looks to be about the optimal solution. For C0, k=8 looks to be optimal. From the combined graph, it's difficult to tell what the optimal K for COCD should be. Evaluating the plots individually may assist.
...
```
From the output above it can be seen that the combination with CO and CD has the highest error. This would lead to not keeping both values in the mix. Give CD by itself has the lowest error this seems the most likely option to address. 
